...
function readLocalUnifiedKey() {
  ...
  try {
    ...
    throw new Error('Cannot read local swordcli configuration. Start its backend first, or set SWORDCLI_BASE_URL and SWORDCLI_TOKEN.');
  } finally { db?.close(); }
}
...
export async function configureSwordBackend(env, readKey = readLocalUnifiedKey) {
  ...
  if (typeof key !== 'string' || !key.trim()) throw new Error('Missing swordcli unified API key. Start the backend or configure SWORDCLI_TOKEN.');
