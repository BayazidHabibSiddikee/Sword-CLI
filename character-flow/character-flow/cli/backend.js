...
//  2. <repo>/swordcli/server/data/freeapi.db  (this file lives at
//     <repo>/character-flow/character-flow/cli/backend.js, so ../../../swordcli)
...
function candidateDbPaths() {
  ...
  list.push(
    ...
    '/home/sword/Documents/Characters/swordcli/server/data/freeapi.db',
  );
  return [...new Set(list)];
}
...
export async function configureSwordBackend(env, readKey = readLocalUnifiedKey, opts = {}) {
  ...
  // Local backends: sword-server (:3101) or the swordcli stack (:3001), both loopback only.
