...
Location: `scraper/` inside the SwordCLI repository (this toolkit is vendored; any
old absolute `/home/...` web-scraper path no longer exists).
Venv: `venv/bin/python` (always use this interpreter — deps are installed there).
Data output dir: resolved by `paths.output_dir()` — override with `SWORD_SCRAPER_OUTPUT`,
defaults to `scraper/output/` inside the project.
