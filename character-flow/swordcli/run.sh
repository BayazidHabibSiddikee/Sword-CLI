#!/bin/bash
# SwordCLI + AI Debate - Unified Startup Script
...
start_swordcli() {
    ...
    echo "Starting SwordCLI Express Server..."
    cd "$SCRIPT_DIR"
    nohup npx tsx server/src/index.ts > "$LOG_DIR/swordcli.log" 2>&1 &
    echo $! > "$PID_DIR/swordcli.pid"
    for i in {1..10}; do
...
stop_all() {
    ...
    [ -f "$PID_DIR/swordcli.pid" ] && kill $(cat "$PID_DIR/swordcli.pid") 2>/dev/null || true
