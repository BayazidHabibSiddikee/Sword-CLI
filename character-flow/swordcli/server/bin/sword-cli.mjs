#!/usr/bin/env node
/**
 * sword-cli — SwordCLI's agentic terminal client.
...
async function main() {
  ...
  console.log(
    `${c.bold('⚔ sword-cli — SwordCLI Agent')}\n` +
  ...
  while (true) {
    ...
    try {
      ...
      if (/fetch failed|ECONNREFUSED/.test(err.message)) {
        console.log(c.dim(`Is the server running at ${BASE}? (start: cd swordcli/server && npm run dev, or pass SWORDCLI_BASE_URL)`));
      }
      if (err.message?.includes('404')) {
        console.log(c.dim(`That server at ${BASE} doesn't have /api/agent routes — it's likely a stale dev server.`));
        console.log(c.dim(`Find it with: ss -ltnp | grep 3001  →  kill <pid>, then restart: cd swordcli/server && npm run dev`));
      }
    }
  }
  rl.close();
...
main().catch((err) => {
  ...
  if (msg.includes('404')) {
    ...
    console.error(c.dim(`Find it: ss -ltnp | grep 3001 → kill <pid>, then restart: cd swordcli/server && npm run dev`));
  }
  process.exit(1);
});
