...
export async function chat({ messages, tools = [], model, preferredProvider, signal, onToken, timeoutMs = 600000 }) {
  ...
  for (const p of ordered) {
    ...
    // Resolve the endpoint FIRST: `native` is needed below to pick the default
    // model, and using it before this const declaration throws a TDZ error that
    // (being outside the try) aborts the whole failover chain.
