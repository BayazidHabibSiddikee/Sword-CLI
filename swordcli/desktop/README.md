...
Lean Electron menu-bar app for [swordcli](https://github.com/tashfeenahmed/swordcli):
...
npm run desktop:dist:win    # Windows → desktop/dist-electron/SwordCLI-…-x64 installer
```
...
- DB + config live in `~/Library/Application Support/SwordCLI/` (macOS) /
  `%APPDATA%/SwordCLI/` (Windows).
