...
app.setName('SwordCLI');
app.setPath('userData', path.join(app.getPath('appData'), 'SwordCLI'));
...
if (!app.requestSingleInstanceLock()) {
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      console.log(`[desktop] SwordCLI running on http://127.0.0.1:${port}`);
